...
def conv_unit(input_layer, num_filter, is_training, name):
    with slim.arg_scope([slim.batch_norm], is_training=is_training):
        with tf.variable_scope("conv_unit_"+name):
            part1 = slim.batch_norm(input_layer, activation_fn=None)
            part2 = tf.nn.relu(part1)
            part3 = slim.conv2d(part2, num_filter, [3,3], activation_fn=None)
            part4 = slim.batch_norm(part3, activation_fn=None)
            part5 = tf.nn.relu(part4)
            output = slim.conv2d(part5, num_filter, [3,3], activation_fn=None)            
    return output
...
def alexnet_v2(inputs,
  ...
  with tf.variable_scope(scope, 'simple_cnn', [inputs]) as sc:
    ...
    with slim.arg_scope([slim.conv2d, slim.fully_connected, slim.max_pool2d],
            ...
            net = conv_unit(inputs, 64, is_training, "1")
            net = slim.max_pool2d(net, [2, 2], 2, scope='pool1')

            # Input image size 64x64
            net = conv_unit(net, 128, is_training, "2")
            net = slim.max_pool2d(net, [2, 2], 2, scope='pool2')

            # Input image size 32x32
            net = conv_unit(net, 256, is_training, "3")
            net = slim.max_pool2d(net, [2, 2], 2, scope='pool3')

            # Input image size 16x16            
            net = conv_unit(net, 256, is_training, "4")
